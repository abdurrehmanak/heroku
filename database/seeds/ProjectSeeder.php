...
class ProjectSeeder extends Seeder
{
    ...
    private function taskBoardColumn()
    {
        ...
        $board2->column_name = 'Done';
        $board2->slug = str_slug($board2->column_name, '_');
        $board2->label_color = '#00c292';
    ...
    private function createTask($faker, $project)
    {
        ...
        $task->due_date = $faker->randomElement([$faker->dateTimeThisMonth($max = 'now'), $faker->dateTimeThisYear($max = 'now')]);
    ...
    private function createInvoice($faker, $project)
    {
        ...
        $invoice->invoice_number = $faker->unique()->numberBetween(14957051661, 14957059661);
        ...
        $payment->paid_on = \Carbon\Carbon::parse($faker->numberBetween(1, 12).'/'.$faker->numberBetween(1, 30).'/'.date('Y'))->format('Y-m-d');
        $payment->save();
