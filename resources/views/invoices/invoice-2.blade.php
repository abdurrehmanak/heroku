        ...
        <section id="memo">
            <div class="logo">
                @if(is_null($global->logo))
                    <img src="{{ public_path().'/img/worksuite-logo.png' }}" />
                @else
            ...
            <section id="client-info">
                @if(!is_null($invoice->project->client))
                    ...
                    @if($invoice->project->client->client_details)
                        ...
                        <div>
                            <span>{!! nl2br($invoice->project->client->client_details->address) !!}</span>
                        </div>
                    @endif
        ...
        @elseif(!is_null($invoice->client_id))
            <section id="client-info">
                @if(!is_null($invoice->client))
                    ...
                    <div>
                        <span>{!! nl2br($invoice->clientdetails->address) !!}</span>
                    </div>
                    @endif
        ...
        @if(is_null($invoice->project) && !is_null($invoice->estimate) && !is_null($invoice->estimate->client))
            <section id="client-info">
                @if(!is_null($invoice->estimate->client))
                    ...
                    <div>
                        <span>{!! nl2br($invoice->estimate->client->client_details->address) !!}</span>
                    </div>
        ...
        <section id="sums">
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                <tr class="amount-total">
                    ...
                    <td>
                        <hr>{!! htmlentities($invoice->currency->currency_symbol)  !!}{{ number_format((float)$invoice->total, 2, '.', '') }}
                    </td>
                </tr>
                @if ($invoice->credit_notes()->count() > 0)
                    <tr>
                        ...
                        <td>
                            <hr>{!! htmlentities($invoice->currency->currency_symbol)  !!}{{ number_format((float)$invoice->appliedCredits(), 2, '.', '') }}
                        </td>
                    </tr>
                @endif
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        {!! htmlentities($invoice->currency->currency_symbol)  !!}{{ number_format((float)$invoice->amountDue(), 2, '.', '') }}</td>
                </tr>
            </table>
